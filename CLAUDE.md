...
```
backend/app/
  api/            — FastAPI routers, one file per resource (projects, models, runs, files, docs, init, env, sql, profiles)
  db/
  ...
  dbt/
    ...
    init_scripts.py — Read/write init/*.sh scripts
    interactive.py — InteractiveInitManager (ptyprocess PTY sessions)
  events/
  ...
  projects/
    ...
    service.py    — Upsert projects to DB
  watcher/
...
All 6 in `backend/app/db/models.py`:
- `projects` — discovered dbt projects
- `init_steps` — ordered init pipeline steps per project
...
- Topics: `project:{id}` (all project events) | `init:{session_id}` (PTY output only)
- SSE helpers (`events/sse.py`) handle subscribe/unsubscribe — prefer them for endpoints
- `sse_response(topic)` — standard project stream with 15s keepalive
...
| `run_log` | project | one line of stdout/stderr |
...
| `init_output` | init | PTY terminal chunk |
| `init_finished` | init | PTY process exited |

### Two Distinct Init Systems

**1. Init Pipeline** — runs dbt deps + custom shell scripts in sequence
- Entry: `POST /api/projects/{id}/open`
- Scripts in `{project_path}/init/*.sh` (managed by `dbt/init_scripts.py`)
- SSE topic: `project:{id}`
- Events: `init_pipeline_started` → `init_step` (×N) → `init_pipeline_finished`

**2. Interactive dbt init** — full PTY terminal
- Entry: `POST /api/projects/init-session/start`
- `manager` singleton in `app.dbt.interactive`
- First pip-installs the adapter, then spawns `dbt init` via ptyprocess
...
```tsx
// Always use useProjectEvents — never poll
...
- SSE via `useProjectEvents` / `useInitSessionEvents` from `lib/sse.ts`
